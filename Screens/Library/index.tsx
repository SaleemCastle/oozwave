...
const soundMatchScore = (aRaw: string, bRaw: string) => {
    ...
    // Simple edit distance capped score
...
const Library: React.FC<LibraryProps> = ({ navigation }) => {
    ...
    const sortedTracks = useMemo(() => {
        ...
        switch (songSort) {
            ...
            case 'recent':
                // Without an explicit date, fall back to id desc as pseudo-recency
    ...
    const renderSearchBar = () => (
        <SearchBar>
            ...
            <TouchableOpacity onPress={() => setSoundMatch((s) => !s)}>
                <McText medium size={11} color={soundMatch ? themeColors.neonMagenta : Colors.grey3}>
                    Sound-Match
                </McText>
            </TouchableOpacity>
        </SearchBar>
    ...
    const renderSortRow = () => (
        <SortRow>
            ...
            <NeonButton style={{ paddingHorizontal: 12, paddingVertical: 8 }} onPress={handleCreatePlaylist}>Create playlist</NeonButton>
        </SortRow>
    )
    ...
    const renderAlbums = () => (
        <FlatList
            ...
            renderItem={({ item }) => (
                <CollectionCard onLongPress={() => setConfirm({ visible: true, title: 'Offline', message: 'Make album available offline?', onConfirm: () => toggleOffline('albums', item.album) })}>
                    <CoverImage //@ts-ignore
    ...
    const renderArtists = () => (
        <FlatList
            ...
            renderItem={({ item }) => (
                <ArtistRow onLongPress={() => setConfirm({ visible: true, title: 'Offline', message: 'Make artist available offline?', onConfirm: () => toggleOffline('artists', item.artist) })}>
                    <AvatarCircle>
...
const EmptyState: React.FC<{ title: string; subtitle?: string; onAction?: () => void }> = ({ title, subtitle, onAction }) => (
    <View style={emptyStyles.container}>
        ...
        {!!onAction && (
            <NeonButton style={{ marginTop: 12 }} onPress={onAction}>Create playlist</NeonButton>
        )}
    </View>
)
